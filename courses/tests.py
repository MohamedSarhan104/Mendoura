...
class CertificateVerificationTests(TestCase):
    ...
    def test_download_falls_back_to_in_memory_pdf_when_stored_copy_cant_be_read(self):
        """Regression test for a real production bug: the PDF uploads to
        storage successfully but reading it back fails (Cloudinary
        rejecting delivery of a PDF stored under the wrong resource type,
        in the real incident). That must never surface as a raw 500 to a
        student clicking "Download Certificate" -- it should silently
        fall back to a freshly-built copy instead."""
        certificate = self._complete_course()
        self.assertTrue(certificate.pdf_file)  # already generated by course completion

        with patch('django.db.models.fields.files.FieldFile.read', side_effect=OSError('storage unreachable')):
            response = self.client.get(reverse('certificate_download', args=[certificate.uuid]))
    ...
    def test_download_falls_back_when_pdf_was_never_generated_and_generation_fails(self):
        certificate = self._complete_course()
        certificate.pdf_file.delete(save=True)
        certificate.refresh_from_db()
        self.assertFalse(certificate.pdf_file)

        with patch('courses.models.Certificate.generate_pdf', side_effect=OSError('cloudinary upload failed')):
            response = self.client.get(reverse('certificate_download', args=[certificate.uuid]))

        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.content.startswith(b'%PDF'))

    def test_verify_search_redirects_to_certificate_on_valid_id(self):
        certificate = self._complete_course()
        response = self.client.get(reverse('certificate_verify_search'), {'q': str(certificate.uuid)})
        self.assertRedirects(response, reverse('certificate_verify_short', args=[certificate.uuid]))

    def test_verify_search_shows_error_on_unknown_id(self):
        import uuid as uuid_module
        response = self.client.get(reverse('certificate_verify_search'), {'q': str(uuid_module.uuid4())})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'No certificate found')

    def test_verify_search_shows_error_on_malformed_id(self):
        response = self.client.get(reverse('certificate_verify_search'), {'q': 'not-a-real-uuid'})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'No certificate found')

    def test_verify_search_page_loads_with_no_query(self):
        response = self.client.get(reverse('certificate_verify_search'))
        self.assertEqual(response.status_code, 200)

    def test_certificate_page_shows_a_clickable_verification_link(self):
        certificate = self._complete_course()
        response = self.client.get(reverse('certificate_verify', args=[certificate.uuid]))
        self.assertContains(response, f'href="{certificates.verification_url(certificate)}"')

    def test_footer_links_to_certificate_verification(self):
        response = self.client.get(reverse('platform_home'))
        self.assertContains(response, reverse('certificate_verify_search'))
...
class QuizCertificateEmailTests(TestCase):
    ...
    def test_certificate_email_still_sends_when_stored_pdf_cant_be_read(self):
        """Regression test for a real production bug: generate_pdf()'s
        upload to storage succeeds, but reading the stored copy back to
        attach it to the email fails (Cloudinary rejecting delivery of a
        PDF stored under the wrong resource type, in the real incident).
        @_never_raises used to swallow that as a logged-only failure --
        the certificate got created but the student's email quietly never
        sent. It must now fall back to an in-memory copy instead."""
        with patch('django.db.models.fields.files.FieldFile.read', side_effect=OSError('storage unreachable')):
            self.client.force_login(self.student)
            self.client.post(reverse('mark_lecture_complete', args=[self.course.id, self.lecture.id]))
...
class ResendCertificateEmailCommandTests(TestCase):
    """python manage.py resend_certificate_email -- the manual recourse for
    a certificate that issued fine but whose email silently failed (see
    QuizCertificateEmailTests.test_certificate_email_still_sends_when_stored_pdf_cant_be_read
    for the bug this exists to recover from)."""

    def setUp(self):
        instructor = User.objects.create_user(username='resend_inst', password='pw', is_instructor=True)
        student = User.objects.create_user(
            username='resend_stud', password='pw', is_student=True, email='resend@example.com')
        track = Track.objects.create(name='Resend Track')
        course = Course.objects.create(
            instructor=instructor, track=track, title='Resend Course', description='...',
            production_type=Course.ProductionType.SCRIPT_ONLY, price=Decimal('0.00'),
            is_free=True, status=Course.Status.PUBLISHED,
        )
        ...
        self.certificate = Certificate.objects.get_or_create(enrollment=enrollment)[0]
        self.certificate.generate_pdf()

    def test_resends_email_for_a_given_certificate(self):
        call_command('resend_certificate_email', '--uuid', str(self.certificate.uuid))
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].to, ['resend@example.com'])
        self.assertEqual(len(mail.outbox[0].attachments), 1)

    def test_raises_on_unknown_uuid(self):
        import uuid as uuid_module
        with self.assertRaises(CommandError):
            call_command('resend_certificate_email', '--uuid', str(uuid_module.uuid4()))

    def test_raises_on_malformed_uuid(self):
        with self.assertRaises(CommandError):
            call_command('resend_certificate_email', '--uuid', 'not-a-uuid')
...
class CourseCreationWizardTests(TestCase):
    """The guided 3-step course creation wizard: Details -> Modules & Content
    (combined: add a module, then immediately add its lessons/quiz, loop
    or move on) -> Review & Submit. Replaces the old "create course, then
    figure out where to go next" flow. Modules & Content used to be two
    separate steps (add every module first, then fill in each one's
    content afterward) -- now adding a module lands the instructor
    straight on that module's own content page instead."""
    ...
    def test_step2_requires_at_least_one_module_before_next_is_offered(self):
        # The enabled "Next: Review" link's real href is only ever
        # server-rendered once a module exists -- checked as an actual
        # anchor tag rather than the raw phrase, since that phrase also
        # lives inside the page's JS (as a fallback string the client
        # rebuilds the block from after an AJAX delete) and is therefore
        # always present in the response body regardless of module count.
        # Not following the "add" POST's redirect (straight into the new
        # module's own content page) is what keeps this test on the
        # modules list page for its second, separate GET below.
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        review_url = reverse('course_wizard_review', args=[course.id])
        ...
        self.assertNotContains(response, f'<a href="{review_url}"')

        self.client.post(reverse('course_wizard_modules', args=[course.id]),
                          {'action': 'add', 'title': 'Module 1', 'order': 1})
        response = self.client.get(reverse('course_wizard_modules', args=[course.id]))
        self.assertContains(response, f'<a href="{review_url}"')
    ...
    def test_advance_next_review_goes_straight_to_review(self):
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        self.client.post(reverse('course_wizard_modules', args=[course.id]),
        ...
        url = reverse('course_wizard_module_content', args=[course.id, module.id])
        response = self.client.post(url, {'action': 'advance', 'next': 'review'})
        self.assertRedirects(response, reverse('course_wizard_review', args=[course.id]))

    def test_advance_next_add_module_loops_back_to_modules_step(self):
        # The core of the new combined-step flow: once a module's content
        # is ready, the instructor explicitly chooses to add another one
        # (looping back to course_wizard_modules, which -- per the "add
        # module redirects into its content" behavior tested elsewhere --
        # immediately lands them on ITS content page too) rather than
        # continuing to the next pre-made module the old flow relied on.
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        self.client.post(reverse('course_wizard_modules', args=[course.id]),
        ...
        url = reverse('course_wizard_module_content', args=[course.id, module.id])
        response = self.client.post(url, {'action': 'advance', 'next': 'add_module'})
        self.assertRedirects(response, reverse('course_wizard_modules', args=[course.id]))

    def test_advance_without_a_next_choice_defaults_to_review(self):
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        self.client.post(reverse('course_wizard_modules', args=[course.id]),
    ...
    def test_add_module_via_ajax_returns_redirect_url_into_its_content_page(self):
        # Adding a module goes straight into that module's own lessons/
        # quiz page now (the combined-step restructuring) instead of
        # staying on the modules list -- an X-Requested-With POST gets a
        # 200 JSON response with a redirect_url for the client to navigate
        # to, rather than inline row HTML to insert in place.
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        response = self.client.post(
    ...
    def test_add_module_without_ajax_redirects_straight_into_its_content_page(self):
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        response = self.client.post(
            reverse('course_wizard_modules', args=[course.id]),
            {'action': 'add', 'title': 'Plain Module', 'order': 1})
        module = Module.objects.get(title='Plain Module')
        self.assertRedirects(
            response, reverse('course_wizard_module_content', args=[course.id, module.id]))

    def test_module_content_page_offers_add_another_module_and_review_choices(self):
        self.client.force_login(self.instructor)
        self.client.post(reverse('create_course'), self._course_details_payload())
        course = Course.objects.get(title='Wizard Course')
        self.client.post(reverse('course_wizard_modules', args=[course.id]),
        ...
        response = self.client.get(
            reverse('course_wizard_module_content', args=[course.id, module.id]))
        self.assertContains(response, 'Add Another Module')
        self.assertContains(response, 'Next: Review')
